...
KZM2_SKIP_LOCK="0"
case "$1" in
    --healthmon-daemon) KZM2_SKIP_LOCK="1" ;;
    --telegram-daemon)
        _self_pid="$$"
...
get_dpi_profile() {
    ...
    case "$p" in
        tt_default|tt_fiber|blockcheck_auto|custom) echo "$p" ;;
...
select_dpi_profile() {
    ...
    for _id in tt_default tt_fiber blockcheck_auto; do
        _num=""
        case "$_id" in
            tt_default) _num="1" ;;
            tt_fiber) _num="2" ;;
...
if [ "$origin" = "auto" ]; then
    ...
    print_line "-"
    printf '%s' "$(T dpi_prompt "Seciminizi yapin (0-3): " "Select an option (0-3): ")"; read -r sel || return 1
    ...
    print_line "-"
    echo "$(T dpi_restart_msg 'DPI profili uygulaniyor, Zapret2 yeniden baslatiliyor...' 'Applying DPI profile, restarting Zapret2...')"
    /opt/etc/init.d/S90-zapret2 restart >/dev/null 2>&1
    _rc=$?
    if [ "$_rc" -eq 0 ]; then
        echo "$(T dpi_restart_ok 'Zapret2 yeniden baslatildi.' 'Zapret2 restarted.')"
    else
        echo "$(T dpi_restart_fail 'UYARI: Zapret2 yeniden baslatilamadi. Komut: /opt/etc/init.d/S90-zapret2 restart' 'WARNING: Could not restart Zapret2. Command: /opt/etc/init.d/S90-zapret2 restart')"
    fi
...
add_ipset_nfqueue_rules() {
    ...
    iptables -t mangle -I POSTROUTING 1 ${WAN:+-o $WAN} -p udp -m multiport --dports 443 \
        ...
        -j NFQUEUE --queue-num 300 --queue-bypass >/dev/null 2>&1
...
update_nfqws_parameters() {
    ...
    AUTO_FULL=""
    if [ "$profile" = "blockcheck_auto" ] && printf '%s' "$AUTO_PARAMS" | grep -q -- '--filter-'; then
        # Blockcheck parser may save a full combined NFQWS2_OPT-style line
        # (HTTP + TLS + QUIC). In that case write it as-is instead of
        # injecting the same auto params into every filter section.
...
_zapret2_iptables_ok() {
    iptables -t mangle -L -n 2>/dev/null | grep -q "NFQUEUE.*300" || \
    iptables -L -n 2>/dev/null | grep -q "NFQUEUE.*300"
}
...
echo "$(tpl_render "$(T TXT_IPV6_WIZARD_START)" VAL "$IPV6_ANSWER")"
    # install_easy.sh zapret2'de TPWS yok, OpenWRT'de LAN/WAN ifindex sorulmaz
    # Soru sirasi (zapret2): guvenlik duvari -> IPv6 -> filtre modu -> nfqws2 -> duzenleme
...
run_zapret2_install_easy() {
    ...
    # confirmation before firewall/IPv6 questions. First try the standard
    # OpenWRT-style sequence, then retry with a leading "y" if needed.
    local _ipv6="${1:-n}"
...
install_zapret2() {
    ...
    # OpenWRT/Entware sistemlerinde LAN/WAN ifindex sorulmaz (select_mode_iface erken donar)
...
blockcheck_test_menu() {
    while true; do
        ...
        case "$ch" in
            ...
            4) kzm2_export_active_dpi_profile ;;
            0) return ;;
...
backup_zapret_settings() {
    # Back up Zapret2 settings (config + key state files) into a tar.gz under BACKUP_BASE/zapret_settings
...
telegram_bot_daemon() {
    ...
    while true; do
        ...
        printf '%s' "$raw" | grep -q '"ok":true' || {
            ...
            # Bu instance kendini kapatsin; watchdog/start tek instance olarak yeniden baslatir.
            if printf '%s' "$raw" | grep -q '"error_code":409'; then
                printf '%s\n' "$(date '+%Y-%m-%d %H:%M:%S') | tgbot | conflict_409 exiting self pid=$$" >> "$TG_BOT_LOG_FILE"
                exit 1
            fi
            sleep "${TG_BOT_POLL_SEC:-5}"
            continue
...
DEOF
    ...
    manual_dpi_get|manualdpi_get)
        ...
        manual_dpi_save|manualdpi_save)
            ...
            if [ -n "$_err" ]; then
                fail "$_err"
            elif ! kzm2_manual_dpi_dryrun "$_opt"; then
                fail "$(kzm2_manual_dpi_msg 'Zapret2 dry-run do&#287;rulamas&#305; ba&#351;ar&#305;s&#305;z. De&#287;i&#351;iklik uygulanmad&#305;' 'Zapret2 dry-run validation failed. Change was not applied')"
...
' | sed '/^[[:space:]]*$/d'
    ...
    # nfqws2 @file must be the only command line argument. Run dry-run
    # directly with the expanded argument list so valid edits are not rejected.
    set -f
    _args="$(cat "$_args_file" 2>/dev/null)"
    rm -f "$_args_file" 2>/dev/null
    # intentional word splitting for nfqws2 args
...
kzm2_manual_dpi_write() {
    ...
    _ts="$(date +%Y%m%d_%H%M%S 2>/dev/null || echo manual)"
    KZM2_MANUAL_DPI_BAK="/opt/zapret2/config.bak_manual_${_ts}"
...
case "$ACTION" in
    ...
    dpi_set)
        ...
        case "$_p" in
            tt_default|tt_fiber|blockcheck_auto) ;;
    ...
    backup_settings)
        _dir="/opt/zapret2_backups/zapret_settings"
        mkdir -p "$_dir" 2>/dev/null
        _ts=$(date +%Y%m%d_%H%M%S 2>/dev/null)
...
function fmtBcCard(S){
  var profileNames={
    'tt_default':'Varsayilan Zapret2 (TTL2 fake)',
...
var V={
  ...
  manualdpi:{title:'Manuel DPI Profili',titleEn:'Manual DPI Profile',sub:'Web Panel &#252;zerinden manuel NFQWS2_OPT stratejisi girin.',subEn:'Enter a custom NFQWS2_OPT strategy from the Web Panel.',html:function(){
    var h='<div class="card wide"><h3>'+(L?'Manual NFQWS2 Profile':'Manuel NFQWS2 Profili')+'</h3>'+
      ...
      '</div>';
    setTimeout(function(){mdpiLoad('config');},100);return h;
  }},
  ...
  backup:{title:'Yedekle / Geri Y&#252;kle',titleEn:'Backup / Restore',sub:'Zapret2 ayarlar&#305; yedekleme ve geri y&#252;kleme.',subEn:'Zapret2 settings backup and restore.',html:function(){
    ...
    return '<div class="bk-grid">'+
      '<div class="card"><h3>&#128190; '+(L?'Backup Zapret2 Settings':'Zapret2 Ayarlar&#305; Yedekle')+'</h3>'+
        ...
        '</div>'+
        '<div class="hint" style="margin-top:8px">'+(L?'Location:':'Konum:')+' /opt/zapret2_backups/zapret_settings/</div>'+
